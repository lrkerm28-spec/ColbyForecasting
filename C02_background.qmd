...
obs = read_observations(scientificname = "Halichoerus grypus")
...
```{r save_model_input}
write_model_input(obsbkg, scientificname = "Halichoerus grypus")
```
...
We have prepared what we call "model inputs", in particular for *Halichoerus grypus*, by thinning observations and selecting background points.  There are lots of other approaches to creating the model input, but for the sake of learning we'll stick with this approach.  We developed a function that will produce our model inputs for a given month, and saved them to disk.
